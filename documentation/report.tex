...
To assess the effectiveness of the new setup, a computer simulation was run based on equation \eqref{spdc_intensity} and the method presented in \cite{spdc-intensity}. The code is written in Python and has been generated with the help of large language models (Claude Fable 5 and Claude Opus 4.8 from Anthropic). It has been manually verified for errors and several sanity checks have been performed, comparing its output to the results given in \cite{spdc-intensity}.

Firstly, equation \eqref{spdc_intensity} has been integrated numerically for values of $-6\degree \leq \theta_s \leq 6\degree$ and $420\ \text{nm} \leq \lambda_s \leq 1000\ \text{nm}$ to recreate FIG. 6 of \cite{spdc-intensity}, using the crystal parameters provided in the paper. The result is shown Figure \ref{fig:fig6_paper}, matching the original. Note that this calculation was performed with the Sellmeier coefficients used in \cite{spdc-intensity}, which are taken from \cite{sellmeier_coeff_used_in_paper}. These are different than those presented in equations \eqref{sellmeier_n_o} and \eqref{sellmeier_n_e} and have been used here to allow for a direct comparison of the figures. The rest of the calculations in this report use the coefficients from equations \eqref{sellmeier_n_o} and \eqref{sellmeier_n_e}, as they are more frequently cited in modern literature.

\begin{figure}[htbp]
\centerline{\includegraphics[scale=0.5]{fig6_paper.png}}
\caption{Angle-resolved spectra of parametric fluorescence}
...
magnetic mounts, half wave plate theory, pics of setup, calculation, theory chsh angles for maximum violation, broad peaks for phase matching measured in practice, dichroic film polarizers?, integral bounds, total coincidence counts after improvisation, link to python, maybe mention parameters from the paper?
