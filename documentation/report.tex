...
\begin{table}[h]
  ...
  \begin{tabular}{lccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Measured count rates (estimation) with focused 20 mW pump laser, integration time 1 s, coincidence window 20 ns}
...
Finally, the CHSH measurement can be performed again with $\alpha = 0^\circ$, $\alpha' = 45^\circ$ on polariser 2 and $\beta = 22.5^\circ$, $\beta' = 67.5^\circ$ on polariser 1 and using equations \eqref{CHSH}, \eqref{CHSH_correlation}, \eqref{CHSH_std_deviation}, \eqref{CHSH_correlation_std_deviation}, \eqref{CHSH_strength}. The results are shown in Table \ref{tab:chsh}, where $C$ denotes the coincidence counts of a single setting and $N$ the sum over the four settings belonging to one correlation coefficient. The final result is $S = 2.681 \pm 0.036$, an 18.6-$\sigma$ violation of Bell's inequality.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l c l c r r c}
    \toprule
    \multicolumn{2}{c}{Polariser 2} & \multicolumn{2}{c}{Polariser 1}
      & & & \\
...
\begin{itemize}
    ...
    \item Gaussian bandpass FWHM: FILTER\_FHWM = 10 nm
